...
class CodeModal(discord.ui.Modal):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if error:
            await interaction.response.send_message("", embed=self._create_error_embed(error))
        else:
            await interaction.response.send_message(": ", embed=self._create_output_embed(output, interaction))

    def _create_error_embed(error: str, author_name: str):
        embed = discord.Embed(
    ...
    def _create_output_embed(output: str, ctx: discord.Interaction):
        embed = discord.Embed(colour=discord.Colour.dark_grey(), timestamp=datetime.datetime.now(datetime.timezone.utc))
        embed.set_author(name=f"{str(ctx.message.author)}'s code execution", icon_url=ctx.message.author.avatar_url)
